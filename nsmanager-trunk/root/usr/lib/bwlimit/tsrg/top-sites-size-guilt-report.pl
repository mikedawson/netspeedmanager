...
#my $cfgSortMethod = "by_times_visited_then_name";
#my $cfgSortMethod = "by_times_visited_then_name";
...
foreach $site_url ( sort ($cfgSortMethod keys (%sites) ) )
{
	...
	if ($cfgNumberToShow > 0)
	{
		...
		{
			print OUTPUT "		<li>" . $sites{$site_url}->{count} ." <a href=\"http://" . $site_url . "/\">" . $site_url . "</a><br/>&nbsp;</li>\n";
		}
		elsif ($cfgSortMethod eq "by_size_then_name")
		{
			print OUTPUT "          <li>" . format_size($sites{$site_url}->{size}) ." <a href=\"http://" . $site_url . "/\">" . $site_url . "</a><br/>&nbsp;</li>\n";
		}
